...
// initialize weather_type to "EXTRASUNNY"
...
void firstOpen()
{
	...
	secondCamCoords.x = 0.0;
	secondCamCoords.y = 0.0;
	secondCamCoords.z = 0.0;

	// set default weather for file
	//strcpy(weather_type, "CLEAR");
}
...
void ScenarioCreator::camera_menu()
{
	...
	// add other lines (4) for the second camera
	...
	// put outside of "while" loop these for initialization only
	//sprintf_s(lines[12], "X			~y~[ %0.1f ]", secondCamCoords.x);
	//sprintf_s(lines[13], "Y			~y~[ %0.1f ]", secondCamCoords.y);
	//sprintf_s(lines[14], "Z			~y~[ %0.1f ]", secondCamCoords.z);
	...
	while (true)
	{
		...
		// Default second camera is off (False) and has the initial coordinates of the main camera
		sprintf_s(lines[10], "Camera 2: %s", secondCam ? "True" : "False");
		sprintf_s(lines[11], "Set camera 2 coord & rot");
		sprintf_s(lines[12], "X			~y~[ %0.1f ]", secondCamCoords.x);
		sprintf_s(lines[13], "Y			~y~[ %0.1f ]", secondCamCoords.y);
		sprintf_s(lines[14], "Z			~y~[ %0.1f ]", secondCamCoords.z);
		...
		else if (bEnter)
		{
			...
			// if enter is pressed on camera 2, change its status (True/False)
			else if (activeLineIndexCamera == 10)
			{	//change status
				secondCam = secondCam ? FALSE : TRUE;
				sprintf_s(lines[10], "Camera 2: %s", secondCam ? "True" : "False");
			}
			else if (activeLineIndexCamera == 11)
			{	//change coordinates and rot fo camera 2
				secondCamCoords = camCoords;
				secondCamRot = camRot;
				//sprintf_s(lines[12], "X			~y~[ %0.1f ]", secondCamCoords.x);
				//sprintf_s(lines[13], "Y			~y~[ %0.1f ]", secondCamCoords.y);
				//sprintf_s(lines[14], "Z			~y~[ %0.1f ]", secondCamCoords.z);
...
{
	const float lineWidth = 250.0;
	// adding Vinhood Boulevard, LEGION SQUARE
	...
	static struct {
		...
		{ "LEGION SQUARE", 189.5f, - 928.2f, 31.8f }
	};
	...
	while (true)
	{
		...
		if (bEnter)
		{
			...
			default:
				...
				// save weather in order to use it in DatasetAnnotator
				strcpy(weather_type, lines[activeLineIndexWeather]);
			}
			waitTime = 200;
		}
		else if (bBack)
...
void ScenarioCreator::spawn_peds(Vector3 pos, int num_ped) {
	...
	//	while (found) {
	//		int hours_seed = TIME::GET_CLOCK_MINUTES();
	//		int minutes_seed = TIME::GET_CLOCK_MINUTES();
	//		int seconds_seed = TIME::GET_CLOCK_SECONDS();
	//		GAMEPLAY::SET_RANDOM_SEED(seconds_seed + minutes_seed*10 + hours_seed*100);
	//		ped[i] = PED::CREATE_RANDOM_PED(pos.x, pos.y, pos.z);
	//		hash = ENTITY::GET_ENTITY_MODEL(ped[i]);
	//		found = std::find(hashes.begin(), hashes.end(), hash) != hashes.end();
	//		debug_file << found << " " << hash << "\n";
	//		WAIT(20);
	//		
	//		if (found == 1) {
	//			PED::DELETE_PED(&ped[i]);
	...
	//	//for (int i = 0; i <= 11; i++) {
	//	//	PED::SET_PED_COMPONENT_VARIATION(ped[i], i, 0, 0, 0);
	//	//}
