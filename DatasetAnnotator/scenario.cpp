...
//define FPS
int FPS = 0;
// initialize weather_type and task
char weather_type[15] = "EXTRASUNNY";
// task has been defined defined as a class member
//char task[15] = "reID";
...
DatasetAnnotator::DatasetAnnotator(std::string _output_path, FILE* _file, std::string _peds_file, int _max_samples, int _is_night, char _task[15])
{
	...
	this->line = "";
	this->log = "";
	// moving this after loading the scenario to set the FPS based on the presence of the second camera
	...
	//loadScenario(file_scenario, weather);
	loadScenario(weather);
...
int DatasetAnnotator::update()
{
	...
	if (moving) {
		...
		//ENTITY::SET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), A.x, A.y, A.z, 0, 0, 0, 1);
		//CAM::SET_CAM_COORD(camera, A.x, A.y, A.z);
		//CAM::SET_CAM_ROT(camera, ped_with_cam_rot.x, ped_with_cam_rot.y, ped_with_cam_rot.z, 2);
		//WAIT(1000);
		//ENTITY::SET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), B.x, B.y, B.z, 0, 0, 0, 1);
		//CAM::SET_CAM_COORD(camera, B.x, B.y, B.z);
		//CAM::SET_CAM_ROT(camera, ped_with_cam_rot.x, ped_with_cam_rot.y, ped_with_cam_rot.z, 2);
		//WAIT(1000);
		// point C not working here
		////ENTITY::SET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), C.x, C.y, C.z, 0, 0, 0, 1);
		////WAIT(1000);
		////ENTITY::SET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), A.x, A.y, A.z, 0, 0, 0, 1);
	...
	for (int i = 0; i < number_of_peds; i++){
		...
		if (ped2cam_distance < MAX_PED_TO_CAM_DISTANCE) {
			...
			for (int n = -1; n < number_of_joints; n++) {
				...
				// save ped hash for identifier
				coords_file << "," << ENTITY::GET_ENTITY_MODEL(peds[i]);			  // pedestrian Hash
...
// added weater argument for possibility of random weather vs predefined weather
//void DatasetAnnotator::loadScenario(const char* fname, char* weather)
void DatasetAnnotator::loadScenario(char* weather)
{
	...
	float speed;
	while (myreadLine(this->file, &pos, &npeds, &ngroup, &currentBehaviour, &speed,
