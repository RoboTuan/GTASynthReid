...
admitted_scenarios = ["airport_entrance", "ferris"]
seeds ={
    "airport_entrance":0,
    "ferris":10
}
...
if scenario.lower() in admitted_scenarios:
    subfolder = scenario.lower()
    folder = base_path + "/" + subfolder
    if not os.path.isdir(folder):
        os.makedirs(folder)
    # Randomize the admitted peds with a random state different from the other scenarios
    rnd_admitted_peds = admitted_peds.sample(frac=1, random_state=seeds[scenario]).reset_index(drop=True)
