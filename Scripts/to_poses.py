...
def main(out_dir_path, n_threads, format):
    ...
    for s in Path('annotations').dirs():
        ...
        # Attention to not include the .DS_Store files
        annotations = [file for file in s.files() if file.basename() != ".DS_Store"]
        print(annotations)
        ...
        for i in range(0, len(annotations), n_threads):
            ...
            for j in range(n_threads):
                # CONTROLLARE SE VA BENE if statement
                # Aggiunto questo per uscire se i+j è maggiore o uguale alla
                # lenght della lista di annotations
                if i+j >= len(annotations):
                    break

                print("j: ", j)
                print(out_dir_path, s.basename(), i+j, annotations[i + j].basename(), annotations[i + j].basename().split('.')[0])
